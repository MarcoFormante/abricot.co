    ...
    return (
        <li key={comment.id} className="mt-5 border border-[#e5e7eb] rounded-[10px] p-5 max-md:flex max-md:flex-col max-md:gap-1 max-sm:px-3  max-sm:pt-3">
                ...
                <div className="float-right flex gap-4 self-end">
                  ...
                  <button className="cursor-pointer" aria-label="modifier le commentaire" onClick={()=>{
                ...
                <div>
                  <form className="mt-[20px] mb-5" onSubmit={(e)=>onEditComment(e,comment.id)}>
                    <div className="w-full relative">
                      <Input styles="pr-25" type="text" name="editComment" label="Modifier le commentaire" value={commentToEdit} onChange={(e)=>setCommentToEdit(e.target.value)} />
